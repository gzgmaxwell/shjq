    ...
    return {
      ...
      searchHandle: [
        ...
        },
        {
          ...
          callback: () => {
            updateDictByKey({
              dictionaryKey: "INK_LORE_COMIC_COMMENT_AUDIT_STATUS",
              dictionaryValue: 1,
            }).then(() => {
    ...
    getvipKeyStatus() {
      fetchList({
        ...
        if (res.data.data?.records && res.data.data?.records.length) {
          this.comicBookStatus = res.data.data.records[0].dictionaryValue;
        }
      });
    },
    getList() {
