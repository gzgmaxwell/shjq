    ...
    <div class="titleBox">
      <div class="title">
        ...
        <el-popover
          ...
          width="150"
          trigger="hover"
          :content="$t('offline_download_Benefits')"
        >
          <span slot="reference" class="el-icon-question question"></span>
        </el-popover>
      </div>
  ...
  watch: {
    ruleForm: {
      handler(val) {
        ...
        if (val.isCommentEnabled || val.isDanmuEnabled || val.isCollectible) {
  ...
  methods: {
    init() {
      memberBenefitsConfigQuery().then((res) => {
        ...
        const item = data.find((v) => v.type === "VIP");
        this.ruleForm = item;
      });
    },
    change(val) {
      if (val) {
        ...
        this.ruleForm.isCollectible = val;
      } else {
        this.ruleForm.isInteractiveEnabled = val;
        this.ruleForm.isCommentEnabled = val;
        this.ruleForm.isDanmuEnabled = val;
