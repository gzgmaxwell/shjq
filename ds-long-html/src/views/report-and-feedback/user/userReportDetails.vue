    ...
    handleSubmit(ruleForm) {
      this.$refs[ruleForm].validate((valid) => {
        if (valid) {
          let handleType;
          if (
            this.ruleForm.objectType == "BULLET" ||
            this.ruleForm.objectType == "COMMENT"
          ) {
            handleType = this.ruleForm.violated == 1 ? "REMOVE" : "NONE";
          }
          if (this.ruleForm.objectType == "USER") {
            handleType = this.ruleForm.violated == 1 ? "WARN" : "NONE";
          }
          const params = {
