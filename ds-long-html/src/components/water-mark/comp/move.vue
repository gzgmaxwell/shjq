...
export default {
  ...
  computed: {
    compWaterMarkType() {
      return (item) => {
        if (item.waterMarkType === ENUM_WMT.addWaterMark) {
          return true;
        } else {
          return false;
        }
    ...
    compuActive() {
      return (item) => {
        if (item.isActive && item.waterMarkType === ENUM_WMT.addWaterMark) {
          return "moveActive";
        } else if (
          item.isActive &&
          item.waterMarkType === ENUM_WMT.delWaterMark
        ) {
          return "moveActiveDel";
        }
        return "";
      };
    },
  ...
  methods: {
    ...
    handleClick() {
      this.$emit("activeClick", this.item);
    },
