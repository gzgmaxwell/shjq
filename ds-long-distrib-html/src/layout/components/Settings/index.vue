...
import { configGet, configSave } from "@/api/common";
...
const changeSysInfo = (val: any) => {
  configSave({
    messageConfig: {
      messageNotice: val,
    },
  }).then(() => {
    settingsStore.changeSetting({ key: "sysInfo", value: val });
  });
};
...
onMounted(() => {
  window.document.body.setAttribute("layout", settingsStore.layout);
  configGet().then((res) => {
    settingsStore.changeSetting({ key: "sysInfo", value: res.data.data?.messageConfig?.messageNotice });
  });
});
</script>
...
<template>
  <div class="settings-container">
    ...
    <div class="py-[8px] flex justify-between">
      <span class="text-xs">开启系统通知</span>
      <el-switch :value="settingsStore.sysInfo" @change="changeSysInfo" />
    </div>
    <div class="py-[8px] flex justify-between">
